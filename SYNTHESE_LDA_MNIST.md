...
Où:
- **$S_b$** (inter-classe) = $\sum_{c=1}^{C} n_c (\mu_c - \mu_{global})^2$ 
  → Maximiser la **séparation** entre moyennes de classes
  
- **$S_w$** (intra-classe) = $\sum_{c=1}^{C} \sum_{x \in c} (x - \mu_c)^2$ 
  → Minimiser la **variance** à l'intérieur de chaque classe
...
Gain: 85% reduction, -1.3% accuracy, 5.5x speedup ✅
```
...
Ratio: 7.1 dimensions supprimées / 1% accuracy perdu
```
...
3️⃣ Performance: 95.2% accuracy avec 9D seulement
   → -86% dimensions, -1.3% accuracy = bon deal
